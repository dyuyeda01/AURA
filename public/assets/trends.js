...
async function loadMaster() {
  const master = await fetchJSON('data/../data/../data/../data/aura_master.json'.replace(/\.\.\//g,''));
  // The above path resolves to /data/aura_master.json at site root (static pages root is /public).
  // If hosting path differs, adjust accordingly.
  return master;
}
...
function filteredHistory(master, {vendor, cve}) {
  ...
  Object.entries(master).forEach(([id, obj]) => {
    ...
    (obj.history||[]).forEach(h => points.push({cve:id, date:h.date, score:h.score, vendor:obj.vendor||''}));
  });
    ...
    window.avgChart = new Chart(ctx1, {
      ...
      options: {
        ...
        scales: {
          ...
          y: { ticks: { color:'#94a3b8' }, grid: { color: 'rgba(148,163,184,0.2)' }, suggestedMin:0, suggestedMax:100 }
        }
      }
    });
